...
1. **Скачивает** клипы из публичного Telegram-канала
   [`@MELLSTROYfonz`](https://t.me/MELLSTROYfonz). По умолчанию
   фильтруются посты, в тексте которых есть слово «Имба» — внутренняя
   маркировка канала для шарящих кусков.
...
- вы хотите перетягивать ассеты из Telegram по расписанию, не держа
  редактор открытым.
...
`videos/`, `images/`, `sounds/`, `particles/`, `text/`) и
переиндексируется после каждого успешного импорта из Telegram.
HTTP-эндпоинты описаны в
[`crates/memstroy-assets-server/src/lib.rs`](crates/memstroy-assets-server/src/lib.rs).

Уровень логов по умолчанию — `info`. Для отладки импорта:
