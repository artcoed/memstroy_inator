...
fn create_template_for_current_clip(state: &mut EditorState) {
    ...
    save_current_template(state);
    state.status = crate::i18n::t("Skeleton template created.").to_string();
}
...
fn point_list_panel(ui: &mut egui::Ui, state: &mut EditorState) {
    ...
    if point_names.is_empty() {
        ui.label(
            RichText::new(crate::i18n::t("No points yet \u{2014} press \"+ Add point\"."))
                .size(11.0)
                .color(COL_TEXT_DIM)
                .italics(),
        );
...
fn point_guide_image_panel(ui: &mut egui::Ui, state: &mut EditorState) {
    ...
    let hovered =
        drag_is_image && pointer_pos.map(|p| zone_rect.contains(p)).unwrap_or(false);
    let bg = if hovered {
    ...
    let caption = if hovered {
        ...
        "Drop another image to replace.".to_string()
    } else {
        format!("Drag an image here for '{}'.", point_name)
    };
    painter.text(
        zone_rect.center(),
        egui::Align2::CENTER_CENTER,
