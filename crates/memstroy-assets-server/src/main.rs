...
struct Cli {
    /// Address to bind the HTTP server on. Defaults to loopback
    /// (`127.0.0.1`) so a local run is not exposed to the LAN; pass
    /// `--addr 0.0.0.0:8765` to listen on all interfaces. When the `PORT`
    /// env var is set (e.g. on Railway / Heroku-style hosts), the server
    /// binds to `0.0.0.0:$PORT` regardless of this flag.
